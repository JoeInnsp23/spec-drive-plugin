...
if [ -d "$SPEC_DRIVE_DIR" ]; then
  echo "⚠️  WARNING: .spec-drive/ already exists"
...
echo "📁 Creating .spec-drive/"
mkdir -p "$SPEC_DRIVE_DIR"

# Create subdirectories
echo "📁 Creating .spec-drive/specs/"
mkdir -p "$SPEC_DRIVE_DIR/specs"

echo "📁 Creating .spec-drive/schemas/v0.1/"
mkdir -p "$SPEC_DRIVE_DIR/schemas/v0.1"

echo "📁 Creating .spec-drive/development/"
...
echo "📁 Creating .spec-drive/logs/"
mkdir -p "$SPEC_DRIVE_DIR/logs"

# Copy schemas from plugin
echo "📋 Copying schemas from plugin..."
if [ -d "$PLUGIN_ROOT/.spec-drive/schemas/v0.1" ]; then
  ...
  echo "  ✅ Copied $(ls -1 "$SPEC_DRIVE_DIR/schemas/v0.1"/*.json 2>/dev/null | wc -l) schemas"
else
  echo "  ⚠️  WARNING: Plugin schemas not found at $PLUGIN_ROOT/.spec-drive/schemas/v0.1"
fi
...
├── index.yaml           # Documentation index (gitignored)
├── specs/               # Spec YAML files (git tracked)
├── schemas/v0.1/        # JSON Schemas for validation (git tracked)
├── development/         # Planning docs (git tracked)
│   ├── current/         # Active version docs
...
- **index.yaml**: AI-optimized index mapping specs → code → tests → docs
...
echo "  ✅ Created .spec-drive/README.md"
...
echo "  ✅ Created .spec-drive/.gitignore"
...
if [ -f "$PROJECT_GITIGNORE" ]; then
  if ! grep -q ".spec-drive/state.yaml" "$PROJECT_GITIGNORE"; then
    ...
    echo "  ✅ Updated $PROJECT_ROOT/.gitignore"
  fi
else
  echo "  ℹ️  No .gitignore found (skipping)"
fi
